...
PnxSite.load = function PnxSite$load() {
    PnxCommon.NsbSite.nsbReady(function() {
        ...
        $(function() {
            ...
            if (!exclude.contains(window.location.hostname)) {
                PnxToolbar.load();
                PnxToolbar.addOrSetButton('access your grids', 'grid48.png', 'http://grids.playnexus.com/');
                PnxToolbar.addOrSetButton('glad hearts game', 'HeartIcon65.png', 'http://gladhearts.playnexus.com/');
...
PnxCommon.FacebookCommon.fbLikeBtn = function PnxCommon_FacebookCommon$fbLikeBtn(id) {
    ...
    /// </param>
    /// <returns type="String"></returns>
    return "<div class='fb-like' data-href='" + Uri.addPair(Uri.join(Config.hostUrl, Config.appPath, 'gladheart.php'), 'popid', id) + "' data-send='false' data-width='450' data-show-faces='false'></div>";
}
...
SudoNsb.Dom.unFocus = function SudoNsb_Dom$unFocus() {
    $("<input type='text' style='position:absolute;top:-1000px;left:-1000px;'/>").appendTo('body').click().focus().remove();
}
SudoNsb.Dom.scrollInViewAw = function SudoNsb_Dom$scrollInViewAw(awp, el) {
    ...
    var ch = SudoNsb.Dom.windowHeight();
    var wb = ch + SudoNsb.Dom.documentScrollTop();
    var eb = el.offset().top + el.outerHeight();
    if (eb > wb) {
        SudoNsb.Dom.scrollToPosAw(awp, eb - ch + 5);
    }
    else {
        awp.done();
    }
...
SudoNsb.PxUser = function SudoNsb_PxUser() {
    /// <field name="_css" type="String" static="true">
    /// </field>
    /// <field name="loginUrl" type="String" static="true">
    /// </field>
    /// <field name="settingsUrl" type="String" static="true">
    /// </field>
...
SudoNsb.PxUser.login = function SudoNsb_PxUser$login(fn, title) {
    ...
    SudoNsb.PxUser._thisUrl = SudoNsb.PxUser.loginUrl;
    SudoNsb.PxUser._loginSub(fn);
}
SudoNsb.PxUser.settings = function SudoNsb_PxUser$settings() {
    SudoNsb.PxUser._thisUrl = SudoNsb.PxUser.settingsUrl;
...
SudoNsb.PxUser._openWin = function SudoNsb_PxUser$_openWin(fn) {
    /// <param name="fn" type="Function">
    /// </param>
    SudoNsb.PxUser._loginWin = window.open(SudoNsb.PxUser._thisUrl || SudoNsb.PxUser.loginUrl, SudoNsb.PxUser.loginWinName, SudoNsb.PxUser.windowFeatures + String.format(',top={0},left={1}', SudoNsb.PxUser.posTop() + 50, SudoNsb.PxUser.posLeft() + 50));
...
SudoNsb.PxUser._openIFrame = function SudoNsb_PxUser$_openIFrame(fn) {
    ...
    iFrame.attr('src', SudoNsb.PxUser._thisUrl || SudoNsb.PxUser.loginUrl);
...
SqlThings.get_thingUrl = function SqlThings$get_thingUrl() {
    /// <value type="String"></value>
    return Uri.join(Config.appPath, 'sql.things.php');
}
...
SqlThings.storeTopic = function SqlThings$storeTopic(topic, thing, secret, id) {
    /// <summary>
...
SqlThings.storeTopicAw = function SqlThings$storeTopicAw(awp, topic, thing, secret, id) {
    /// <summary>
...
SqlThings.storeThingAw = function SqlThings$storeThingAw(awp, td) {
    ...
    new SudoNsb.Await().addAw(BrowserUser.waitSessionLockAw).addDl(function(aw) {
        var more = td.more;
        delete td.more;
        SqlThings.rawTransactAw(aw, { userId: Snsb.get_masterId(), secret: td.secret }, 'insert', td);
        if (more != null) {
            td.more = more;
        }
    }).addDx(function(aw) {
...
SqlThings.removeThingAw = function SqlThings$removeThingAw(awp, mpx) {
    ...
    new SudoNsb.Await().addAw(BrowserUser.waitSessionLockAw).addDl(function(aw) {
        var keys = { id: id, userId: Snsb.get_masterId() };
...
SqlThings.rawTransactAw = function SqlThings$rawTransactAw(awp, keys, transact, content) {
    ...
    if (keys['userId'] == null || keys['userId'] === 'null' || String.isNullOrEmpty(keys['userId'])) {
        debugger;
    }
    if (keys['secret'] && !Snsb.get_isPxUser()) {
...
SudoNsb.ToolTips = function SudoNsb_ToolTips() {
    /// <field name="_css" type="String" static="true">
    /// </field>
    /// <field name="_tip" type="jQueryObject" static="true">
    /// </field>
}
SudoNsb.ToolTips.addTo = function SudoNsb_ToolTips$addTo(sel) {
    ...
    sel.on('mouseenter', function() {
        var ths = SudoNsb.ToolTips._tip = $(this);
        var title = ths.attr('title');
        ths.data('tipText', title).removeAttr('title');
        $("<p class='tooltip AboveHider'></p>").html(title).appendTo('body').fadeIn(400);
    }).on('mouseleave', function() {
        SudoNsb.ToolTips._tip = null;
        var ths = $(this);
...
SudoNsb.ToolTips.clearTips = function SudoNsb_ToolTips$clearTips() {
    if (SudoNsb.ToolTips._tip != null) {
        SudoNsb.ToolTips._tip.attr('title', SudoNsb.ToolTips._tip.data('tipText'));
        $('.tooltip').remove();
        SudoNsb.ToolTips._tip = null;
    }
}
...
FbApi.get_id = function FbApi$get_id() {
    /// <value type="String"></value>
    return FbApi._getSudoId(FbApi._authResponse.userID);
}
...
BrowserUser = function BrowserUser() {
    ...
    /// </field>
    /// <field name="_getSessionUrl" type="String" static="true">
    /// </field>
...
BrowserUser.load = function BrowserUser$load() {
    ...
    BrowserUser._getSessionUrl = Uri.join(Config.appPath, 'file.session.php');
...
BrowserUser.loginAw = function BrowserUser$loginAw(awp) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    if (BrowserUser.session.admin) {
        BrowserUser.adminPop();
        return;
    }
    new SudoNsb.Pop().addAw(SudoNsb.Pop.okCancelAw, function(po) {
        po.msg = "password <input id='BrowserUserPw' type='password'>";
        if (BrowserUser.session.admin) {
            po.msg = "<h3 style='text-align:center'>you are currently admin</h3>" + po.msg;
        }
        po.okFn = function() {
            var pw = $('#BrowserUserPw').val().trim();
            new SudoNsb.Await().addAw(BrowserUser.adminLoginAw, pw).addDx(function() {
                if (BrowserUser.session.admin) {
                    BrowserUser.adminPop();
                }
            }).commit();
        };
        po.focusTo = '#BrowserUserPw';
    }).commit(awp);
}
...
BrowserUser.reloadPage = function BrowserUser$reloadPage() {
...
BrowserUser.retrieveSession = function BrowserUser$retrieveSession(fn) {
    ...
    $.getJSON(Uri.addPair(BrowserUser._getSessionUrl, 'adminPassword', BrowserUser.get_adminPassword()), function(data) {
...
BrowserUser.adminLoginAw = function BrowserUser$adminLoginAw(awp, password) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="password" type="String">
    /// </param>
    BrowserUser.set_adminPassword(password);
    BrowserUser.retrieveSession(function(data) {
        var ses = data;
        if (ses.admin) {
            BrowserUser.session.admin = true;
        }
        else {
            BrowserUser.set_adminPassword(null);
            BrowserUser.session.admin = false;
...
BrowserUser.adminPop = function BrowserUser$adminPop() {
    if (Project.AdminPop) {
        new Project.AdminPop();
    }
    else {
        SudoNsb.Pop.ok("<h3 style='text-align: center'>you are now admin!</h3>");
    }
}
...
SudoNsb.CloudMail.get__sendUrl = function SudoNsb_CloudMail$get__sendUrl() {
    /// <value type="String"></value>
    return Uri.join(Config.appPath, 'sendmail.php');
}
...
SudoNsb.TinyMce = function SudoNsb_TinyMce() {
    /// <field name="_tinyMceUrl" type="String" static="true">
    /// </field>
    /// <field name="_tinyMcejQueryUrl" type="String" static="true">
    /// </field>
    /// <field name="_loaded" type="Boolean" static="true">
    /// </field>
    /// <field name="simpleEdit" type="Object" static="true">
    /// </field>
    /// <field name="listEdit" type="Object" static="true">
    /// </field>
    /// <field name="fullEdit" type="Object" static="true">
    /// </field>
}
SudoNsb.TinyMce.preload = function SudoNsb_TinyMce$preload() {
    new SudoNsb.Await().loadScript('http://tinymce.cachefly.net/4.0/tinymce.min.js', true).loadScript('http://tinymce.cachefly.net/4.0/jquery.tinymce.min.js', true).waitDx(function() {
        return window.tinymce;
    }).addDx(function() {
        SudoNsb.TinyMce.listPlugin();
    }).addDx(function() {
        SudoNsb.TinyMce._loaded = true;
    }).commit();
}
SudoNsb.TinyMce.initAw = function SudoNsb_TinyMce$initAw(awp, selector, options) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    ...
    /// </param>
    options = options || SudoNsb.TinyMce.fullEdit;
    var opts = $.extend({}, options);
    var id = (selector.substr(0, 1) === '#') ? selector.substr(1) : selector;
    new SudoNsb.Await().waitDx(function() {
        return SudoNsb.TinyMce._loaded;
    }).addAw(SudoNsb.TinyMce._bailoutAw, '1').addDx(function() {
        $(selector).tinymce(opts);
    }).waitDx(function() {
        return SudoNsb.TinyMce.getEditor(id) != null;
    }).addAw(SudoNsb.TinyMce._bailoutAw, '2').commit(awp);
}
SudoNsb.TinyMce._bailoutAw = function SudoNsb_TinyMce$_bailoutAw(awp, msg) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="msg" type="String">
    /// </param>
    if (!awp.get_result()) {
        SudoNsb.Pop.ok('not able to load TinyMCE console<br>from 3rd party site<br><i>try reloading the page</i>');
        Inform.error('TinyMce.BailoutAw [{0}]', msg);
        awp.abort();
        return;
    }
    awp.done();
}
SudoNsb.TinyMce.remove = function SudoNsb_TinyMce$remove(selector) {
    /// <param name="selector" type="String">
    /// </param>
    tinymce.remove(selector);
}
SudoNsb.TinyMce.intialized = function SudoNsb_TinyMce$intialized(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Boolean"></returns>
    return tinymce.get(editorId).initialized;
}
SudoNsb.TinyMce.clear = function SudoNsb_TinyMce$clear(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Boolean"></returns>
    return tinymce.get(editorId).setContent('');
}
SudoNsb.TinyMce.insert = function SudoNsb_TinyMce$insert(html, editorId) {
    /// <param name="html" type="String">
    /// </param>
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Boolean"></returns>
    return tinymce.get(editorId).insertContent(html);
}
SudoNsb.TinyMce.mceElements = function SudoNsb_TinyMce$mceElements(selector, editorId) {
    /// <summary>
    /// Returns a jQuery list of elements from an editor.
    /// </summary>
    /// <param name="selector" type="String">
    /// The selector matching elements to find.
    /// </param>
    /// <param name="editorId" type="String">
    /// The editor id or if not given then the active editor.
    /// </param>
    /// <returns type="jQueryObject"></returns>
    try {
        var eo = (editorId != null) ? $(selector, tinymce.get(editorId).getBody()) : $(selector, tinymce.activeEditor.getBody());
        return eo;
    }
    catch ($e1) {
        debugger;
...
SudoNsb.TinyMce.mceContainer = function SudoNsb_TinyMce$mceContainer(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="jQueryObject"></returns>
    return $(tinymce.get(editorId).getContainer());
}
SudoNsb.TinyMce.mceEditArea = function SudoNsb_TinyMce$mceEditArea(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="jQueryObject"></returns>
    return $(tinymce.get(editorId).getContainer()).find('div.mce-edit-area');
}
SudoNsb.TinyMce.getEditor = function SudoNsb_TinyMce$getEditor(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Object"></returns>
    try {
        ...
        Inform.error('failed to get TinyMCE editor {0}', editorId);
        throw ex;
    }
}
SudoNsb.TinyMce.close = function SudoNsb_TinyMce$close(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Object"></returns>
    return tinymce.get(editorId).remove();
}
SudoNsb.TinyMce.isDirty = function SudoNsb_TinyMce$isDirty(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Boolean"></returns>
    return tinymce.get(editorId).isDirty();
}
SudoNsb.TinyMce.clearDirty = function SudoNsb_TinyMce$clearDirty(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    var ed = tinymce.get(editorId);
    ed.startContent = tinymce.trim(ed.getContent({format : 'raw', no_events : 1}));
}
SudoNsb.TinyMce.focus = function SudoNsb_TinyMce$focus(editorId) {
    /// <param name="editorId" type="String">
    /// </param>
    /// <returns type="Object"></returns>
    return tinymce.get(editorId).focus();
}
SudoNsb.TinyMce.cursorToEnd = function SudoNsb_TinyMce$cursorToEnd(ed) {
    /// <param name="ed" type="Object">
    /// </param>
    if (typeof(ed) === 'string') {
        ed = SudoNsb.TinyMce.getEditor(ed);
    }
    
ed.selection.select(ed.getBody(), true); // ed is the editor instance
ed.selection.collapse(false);;
}
SudoNsb.TinyMce.listPlugin = function SudoNsb_TinyMce$listPlugin() {
    /// <summary>
    /// Plugin to force TinyMCE to only edit lists.
    /// plugin: forcelist
    /// settings:
    /// forcelist_container = "ol|ul" (default to ol)
    /// forcelist_nowrap = true|false (true prevents wrapping unwrapped lists on load)
    /// </summary>
    
tinymce.PluginManager.add('forcelist', function(editor, url) {
    editor.on('keydown', function(e) {
        if (e.keyCode != 13) return;
        var content = editor.getContent({format: 'raw'});
        if (content.indexOf('<li><br data-mce-bogus') != -1 || content.indexOf('<li></li>') != -1)
            e.preventDefault();
        //console.log(editor.getContent({format: 'raw'}));
    });
    editor.on('BeforeSetContent', function(e) {
        //console.log(editor.settings.valid_elements);
        // use setting forcelist_container or default to ordered list for unwrapped list
        //console.log('1:' + e.content);
        var lt = editor.settings.forcelist_container || 'ol';
        var content = e.content.replace(/^\s+|\s+$/g, '') || '<'+lt+'>\r\n<li></li>\r\n</'+lt+'>';
        content = e.initial ? '<'+lt+'>\r\n<li></li>\r\n</'+lt+'>' : content;
        var ft = ':'+(content.substr(0,4) || 'not')+':';
        // if not wrapped in a list then wrap it in a list
        if (':<ol>:<ul>:'.indexOf(ft) == -1 && !editor.settings.forcelist_nowrap)
            e.content = '<'+lt+'>\r\n<li>'+ content+'</li>\r\n</'+lt+'>';
        else
            e.content = content;
    ...
    editor.on('GetContent', function(e) {
        // trim off any empty list items
        e.content = e.content.replace(/<li>\s*<\/li>\s*/ig, '');
        // wipe out an empty list
        e.content = e.content.replace(/(<ol>|<ul>)\s*(<\/ol>|<\/ul>)/ig, '');
    });
});
}
...
SudoNsb.Metrics.documentWidth = function SudoNsb_Metrics$documentWidth() {
    /// <returns type="Number" integer="true"></returns>
    return $($(document)).width();
}
SudoNsb.Metrics.documentHeight = function SudoNsb_Metrics$documentHeight() {
    /// <returns type="Number" integer="true"></returns>
...
SudoNsb.Pop = function SudoNsb_Pop() {
    /// <field name="_classCss$1" type="String" static="true">
    /// </field>
    /// <field name="popKey" type="String" static="true">
    /// </field>
    /// <field name="_popList$1" type="Array" static="true">
    /// </field>
    /// <field name="id" type="String">
    /// </field>
    /// <field name="_myBindClass$1" type="String">
    /// </field>
    /// <field name="_noOpt$1" type="Function">
    /// </field>
    /// <field name="closed" type="Boolean">
    /// </field>
    /// <field name="element" type="PositionObject">
    /// </field>
    /// <field name="msg" type="jQueryObject">
    /// </field>
    /// <field name="opt" type="SudoNsb.PopOptions">
    /// </field>
    /// <field name="splash" type="jQueryObject">
    /// </field>
    /// <field name="_awp$1" type="SudoNsb.Await">
    /// </field>
    /// <field name="_success$1" type="Boolean">
    /// </field>
    this._myBindClass$1 = '.' + Snsb.get_newId();
    this._noOpt$1 = function(){};
    SudoNsb.Pop.initializeBase(this);
    this.id = 'Id' + Snsb.get_newId();
    this.opt = new SudoNsb.PopOptions(this);
    MyCss.addStyleOnce((SudoNsb.Pop).get_fullName(), "\r\ndiv.PnxPop {\r\n    position: absolute;\r\n    border: none;\r\n    padding: 0;\r\n    background: #DDD;\r\n    width: 450px;\r\n    min-height: 200px;\r\n\t-webkit-border-radius: 7px;\r\n\t-moz-border-radius: 7px;\r\n\tborder-radius: 7px;\r\n    -moz-box-sizing: border-box;\r\n    -webkit-box-sizing: border-box;\r\n    box-sizing: border-box;\r\n    font-family: 'lucida grande', tahoma, verdana, arial, sans-serif;\r\n    font-size: 12pt;\r\n    color: black;\r\n    border: 8px solid rgb(156,156,156);\r\n}\r\ndiv.PnxPop div.NsbPopMsg {\r\n    margin: 24px;\r\n}\r\ndiv.PnxPop.PnxPopHasButtons {\r\n    padding: 24px 24px 70px 24px;\r\n}\r\ndiv.PnxPop button.OkBtn, div.PnxPop button.CancelBtn {\r\n    position: absolute;\r\n    width: 100px;\r\n    bottom: 25px;\r\n}\r\ndiv.PnxPop button.OkBtn {\r\n    right: 80px;\r\n}\r\ndiv.PnxPop button.CancelBtn {\r\n    left: 80px;\r\n}\r\ndiv.PnxPop.PnxPopOk {\r\n    padding: 30px 65px 70px 65px;\r\n    text-align: center;\r\n}\r\ndiv.PnxPop img.Closer {\r\n    position: absolute;\r\n    width: 24px;\r\n    height: 24px;\r\n    cursor: pointer;\r\n    top: -21px;\r\n    right: -21px;\r\n}\r\ndiv.PnxPop img.Closer.Dim {\r\n    opacity: 1;\r\n}\r\ndiv.PnxPop div.NsbPopHandle {\r\n    height: 12px;\r\n    margin: 0;\r\n    padding: 0;\r\n    background-color: rgba(128,128,128,.25);\r\n    border-bottom: 1px solid rgba(0,0,0,.25);\r\n    -webkit-border-top-left-radius: 19px;\r\n    -webkit-border-top-right-radius: 19px;\r\n    -moz-border-radius-topleft: 19px;\r\n    -moz-border-radius-topright: 19px;\r\n    border-top-left-radius: 19px;\r\n    border-top-right-radius: 19px;\r\n}\r\ndiv.PnxPopSplash {\r\n    position: absolute;\r\n    background: transparent;\r\n}\r\n\r\ndiv.PnxPop.NsbPopInfo {\r\n    display: inline-block;\r\n    font-family: arial;\r\n    font-size: 9pt;\r\n    text-align: left;\r\n    min-height: 100px;\r\n    border: 1px solid grey;\r\n\t-webkit-border-radius: 7px;\r\n\t-moz-border-radius: 7px;\r\n\tborder-radius: 7px;\r\n    -webkit-box-shadow: 3px 3px 18px rgba(50, 50, 50, 0.85);\r\n    -moz-box-shadow:    3px 3px 18px rgba(50, 50, 50, 0.85);\r\n    box-shadow:         3px 3px 18px rgba(50, 50, 50, 0.85);\r\nbackground: #feffe8; /* Old browsers */\r\nbackground: -moz-linear-gradient(top, #feffe8 0%, #d6dbbf 100%); /* FF3.6+ */\r\nbackground: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#feffe8), color-stop(100%,#d6dbbf)); /* Chrome,Safari4+ */\r\nbackground: -webkit-linear-gradient(top, #feffe8 0%,#d6dbbf 100%); /* Chrome10+,Safari5.1+ */\r\nbackground: -o-linear-gradient(top, #feffe8 0%,#d6dbbf 100%); /* Opera 11.10+ */\r\nbackground: -ms-linear-gradient(top, #feffe8 0%,#d6dbbf 100%); /* IE10+ */\r\nbackground: linear-gradient(to bottom, #feffe8 0%,#d6dbbf 100%); /* W3C */\r\nfilter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#feffe8', endColorstr='#d6dbbf',GradientType=0 ); /* IE6-9 */\r\n}\r\ndiv.PnxPop.NsbPopInfo div.NsbPopMsg {\r\n    margin: 4px 12px;\r\n}\r\ndiv.PnxPop.NsbPopInfo div.NsbPopHandle {\r\n    -webkit-border-top-left-radius: 6px;\r\n    -webkit-border-top-right-radius: 6px;\r\n    -moz-border-radius-topleft: 6px;\r\n    -moz-border-radius-topright: 6px;\r\n    border-top-left-radius: 6px;\r\n    border-top-right-radius: 6px;\r\n}\r\n        ");
    this.element = $("<div id='" + this.id + "' class='PnxPop XPnxPop' style='position:absolute;top:-1000px;left:-1000px;'/>").appendTo(document.body).data('PopInstance', this).hide();
}
SudoNsb.Pop.getPop = function SudoNsb_Pop$getPop(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    /// <returns type="SudoNsb.Pop"></returns>
    return $(e.target).closest('.XPnxPop').data('PopInstance');
}
SudoNsb.Pop.info = function SudoNsb_Pop$info(fn) {
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.infoAw(SudoNsb.Await.get_asyncAw(), fn);
}
SudoNsb.Pop.ok = function SudoNsb_Pop$ok(fn) {
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.okAw(SudoNsb.Await.get_asyncAw(), fn);
}
SudoNsb.Pop.okCancel = function SudoNsb_Pop$okCancel(fn) {
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.okCancelAw(SudoNsb.Await.get_asyncAw(), fn);
}
SudoNsb.Pop.infoAw = function SudoNsb_Pop$infoAw(awp, fn) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.aw(awp, function(po) {
        po.set(SudoNsb.Pop.common).set(function() {
            po.closer = true;
            po.pop.element.addClass('NsbPopInfo');
        }).set(fn);
    });
}
SudoNsb.Pop.okAw = function SudoNsb_Pop$okAw(awp, fn) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.aw(awp, function(po) {
        po.set(SudoNsb.Pop.common).setOk(true).set(fn);
    });
}
SudoNsb.Pop.okCancelAw = function SudoNsb_Pop$okCancelAw(awp, fn) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="fn" type="System.Action`1">
    /// </param>
    SudoNsb.Pop.aw(awp, function(po) {
        po.set(SudoNsb.Pop.common).setOk(true).setCancel(true).set(fn);
    });
}
SudoNsb.Pop.aw = function SudoNsb_Pop$aw(awp, fn) {
    /// <param name="awp" type="SudoNsb.Await">
    /// </param>
    /// <param name="fn" type="System.Action`1">
    /// </param>
    var pop = new SudoNsb.Pop();
    pop._awp$1 = awp;
    var po = pop.opt;
    po.set(fn);
    pop.configure();
}
SudoNsb.Pop.common = function SudoNsb_Pop$common(po) {
    /// <param name="po" type="SudoNsb.PopOptions">
    /// </param>
    po.hider = true;
    po.classes = 'PnxPopOk';
}
SudoNsb.Pop.closeAll = function SudoNsb_Pop$closeAll(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    var $enum1 = ss.IEnumerator.getEnumerator(SudoNsb.Pop._popList$1.clone());
    while ($enum1.moveNext()) {
        var pop = $enum1.current;
        pop.close();
    }
}
SudoNsb.Pop.prototype = {
    id: null,
    closed: false,
    element: null,
    msg: null,
    ...
    close: function SudoNsb_Pop$close() {
        /// <summary>
        /// Closes the popup returning _success value to any awaiter.
        /// </summary>
        if (this.closed) {
            return;
        }
        if (this._success$1 && this.opt.okFn != null) {
            this.opt.okFn(null);
        }
        else if (!this._success$1 && this.opt.cancelFn != null) {
            this.opt.cancelFn(null);
        }
        if (this.opt.closeFn != null) {
            this.opt.closeFn(null);
        }
        if (this.opt.preventOk && this._success$1) {
            this.opt.preventClose = this.opt.preventOk = false;
            return;
        }
        if (this.opt.preventClose) {
            this.opt.preventClose = this.opt.preventOk = false;
            return;
        }
        this.closed = true;
        $(document).unbind(this._myBindClass$1);
        SudoNsb.Pop._popList$1.remove(this);
        this.element.remove();
        if (this.splash != null) {
            this.splash.remove();
        }
        ...
        this.element.removeData('PopInstance');
        this.opt.pop = null;
        if (this._awp$1 != null) {
            this._awp$1.done(this._success$1);
        }
    },
    
    _oOk$1: function SudoNsb_Pop$_oOk$1(e) {
        /// <param name="e" type="jQueryEvent">
        /// </param>
        this._success$1 = true;
        this.close();
    },
    
    cancel: function SudoNsb_Pop$cancel(e) {
        /// <param name="e" type="jQueryEvent">
        /// </param>
        this._success$1 = false;
        this.close();
    },
    
    configure: function SudoNsb_Pop$configure(fn) {
        /// <param name="fn" type="System.Action`1">
        /// </param>
        /// <returns type="SudoNsb.Pop"></returns>
        if (fn != null) {
        ...
        this.opt.mobile = $(window).width() < 360;
        if (this.opt.glass) {
            Surface.glass((this.opt.noGlassClose) ? null : ss.Delegate.create(this, this.cancel));
            this.element.appendTo(document.body).addClass('AboveGlass');
        }
        if (this.opt.hider) {
            Surface.hider(true);
            this.element.appendTo(document.body).addClass('AboveHider');
        }
        if (this.opt.width > 0) {
            this.element.css({ width: this.opt.width });
        }
        if (this.opt.ok) {
            this.element.addClass('PnxPopHasButtons');
            $("<button class='OkBtn'/>").appendTo(this.element).html(this.opt.okFace || 'Ok').click(ss.Delegate.create(this, this._oOk$1));
        }
        if (this.opt.cancel) {
            this.element.addClass('PnxPopHasButtons');
            $("<button class='CancelBtn'/>").appendTo(this.element).html(this.opt.cancelFace || 'Cancel').click(ss.Delegate.create(this, this.cancel));
        }
        if (this.opt.closer) {
            $("<img class='Closer Dim' title='close'/>").appendTo(this.element).attr('src', this.opt.closeBtnFace).mouseenter(function(e) {
                $(e.target).removeClass('Dim');
            }).mouseleave(function(e) {
                $(e.target).addClass('Dim');
            }).click(ss.Delegate.create(this, this.cancel));
        }
        if (this.opt.id != null) {
            this.element.attr('Id', this.opt.id);
        }
        if (this.opt.noPnxPopClass) {
            this.element.removeClass('PnxPop');
        }
        if (this.opt.classes != null) {
            this.element.addClass(this.opt.classes);
        }
        if (this.opt.css != null) {
            this.element.css(this.opt.css);
        }
        if (this.opt.style != null) {
            MyCss.addStyleOnce(this.opt.style);
        }
        if (this.opt.msg != null) {
            this.msg = $("<div class='NsbPopMsg'/>").html(this.opt.msg).prependTo(this.element);
        }
        $(document).bind('keydown' + this._myBindClass$1, ss.Delegate.create(this, this._keyDownEv$1));
        this.element.css({ opacity: 0.01 }).show();
        if (this.opt.mobile) {
            this.element.css({ top: 0, left: 0 });
            $(window).scrollTop(0);
        }
        else {
            if (!this.opt.top && !this.opt.left) {
                this.element.position({ my: 'center top', at: 'center top+150', of: window.self });
            }
            else {
                this.element.css({ top: this.opt.top, left: this.opt.left });
            }
        }
        this.element.css({ opacity: '' }).hide();
        if (!!this.opt.fade) {
            this.element.fadeIn(this.opt.fade);
        }
        else {
            this.element.show();
        }
        if (this.opt.focusTo != null) {
            Snsb.defer(ss.Delegate.create(this, function() {
                $(this.opt.focusTo).click().focus();
            }));
        }
        if (!this.opt.noAddList) {
            SudoNsb.Pop._popList$1.add(this);
        }
        if (this.opt.modifyFn != null) {
            this.opt.modifyFn(this.opt);
        }
        return this;
    },
    
    _keyDownEv$1: function SudoNsb_Pop$_keyDownEv$1(e) {
        /// <param name="e" type="jQueryEvent">
        /// </param>
        switch (e.which) {
            case 13:
                if (!this.opt.noRetKey) {
                    Snsb.cancelEvent(e);
                    this.close();
                }
                break;
            case 27:
                if (!this.opt.noEscKey) {
                    Snsb.cancelEvent(e);
                    this.cancel();
                }
                break;
    ...
    _okFn$1: function SudoNsb_Pop$_okFn$1() {
        /// <returns type="Function"></returns>
        var okFn = this.opt.okFn;
        if (okFn != null || this.opt.ok) {
            this.opt.ok = true;
    ...
    _cancelFn$1: function SudoNsb_Pop$_cancelFn$1() {
        /// <returns type="Function"></returns>
        var cancelFn = this.opt.cancelFn;
        if (cancelFn != null || this.opt.cancel) {
    ...
    closeOthers: function SudoNsb_Pop$closeOthers() {
        var $enum1 = ss.IEnumerator.getEnumerator(SudoNsb.Pop._popList$1.clone());
        while ($enum1.moveNext()) {
            var pop = $enum1.current;
            if (pop !== this) {
    ...
    addSplash: function SudoNsb_Pop$addSplash(top, right, bottom, left) {
        ...
        /// </param>
        /// <returns type="jQueryObject"></returns>
        ss.Debug.assert(this.splash == null, 'Splash not null');
        var p = this.element.offset();
        this.splash = $("<div class='PnxPopSplash'/>").css({ top: p.top - top, left: p.left - left, 'padding-top': top, 'padding-right': right, 'padding-bottom': bottom, 'padding-left': left });
        if (this.opt.hider) {
            this.splash.addClass('AboveHider');
        }
        if (this.opt.glass) {
            this.splash.addClass('AboveGlass');
        }
        if (this.opt.classes != null) {
            this.splash.addClass(this.opt.classes);
        }
        this.element.css({ position: 'relative', top: 0, left: 0 });
        this.splash.insertBefore(this.element);
        this.element.appendTo(this.splash);
        return this.splash;
    },
    
    addAw: function SudoNsb_Pop$addAw(fn, arg1, arg2, arg3, arg4) {
        /// <param name="fn" type="System.Action`5">
        /// </param>
        /// <param name="arg1" type="Object">
        /// </param>
        ...
        /// </param>
        /// <returns type="SudoNsb.Await"></returns>
        return SudoNsb.Pop.callBaseMethod(this, 'addAw', [ fn, arg1, arg2, arg3, arg4 ]);
    }
}
...
SudoNsb.PopOptions = function SudoNsb_PopOptions(pop) {
    /// <param name="pop" type="SudoNsb.Pop">
    /// </param>
    ...
    /// </field>
    /// <field name="closeBtnFace" type="String">
    /// </field>
    /// <field name="closeFn" type="Function">
    /// </field>
    /// <field name="closer" type="Boolean">
    /// </field>
    /// <field name="css" type="Object">
    /// Css in dictionary format applied to the whole element
    /// </field>
    /// <field name="dragBar" type="Boolean">
    /// </field>
    /// <field name="draggable" type="Boolean">
    /// </field>
    /// <field name="fade" type="Number" integer="true">
    /// </field>
    /// <field name="focusTo" type="String">
    /// </field>
    /// <field name="glass" type="Boolean">
    /// </field>
    /// <field name="hider" type="Boolean">
    /// </field>
    /// <field name="id" type="String">
    /// </field>
    ...
    /// </field>
    /// <field name="modifyFn" type="System.Action`1">
    /// </field>
    /// <field name="msg" type="String">
    /// </field>
    /// <field name="noAddList" type="Boolean">
    /// </field>
    /// <field name="noEscKey" type="Boolean">
    /// </field>
    /// <field name="noGlassClose" type="Boolean">
    /// </field>
    /// <field name="noPnxPopClass" type="Boolean">
    /// </field>
    /// <field name="noRetKey" type="Boolean">
    /// </field>
    ...
    /// </field>
    /// <field name="pop" type="SudoNsb.Pop">
    /// </field>
    /// <field name="preventClose" type="Boolean">
    /// </field>
    /// <field name="preventOk" type="Boolean">
    /// </field>
    /// <field name="style" type="String">
    /// Full style sheet where .PnxPop will be replaced by the current #Id
    /// Example: div.PnxPop button.OkBtn becomes div#[Id] button.OkBtn
    /// </field>
    /// <field name="top" type="Number" integer="true">
    /// </field>
    /// <field name="width" type="Number" integer="true">
    /// </field>
    this.closeBtnFace = Uri.join(Config.imgPath2, 'closebtn34.png');
    this.pop = pop;
}
SudoNsb.PopOptions.prototype = {
    cancel: false,
    cancelFace: null,
    cancelFn: null,
    classes: null,
    ...
    setOk: function SudoNsb_PopOptions$setOk(ic) {
        /// <param name="ic" type="Boolean">
        /// </param>
        /// <returns type="SudoNsb.PopOptions"></returns>
        this.ok = true;
        return this;
    },
    
    setCancel: function SudoNsb_PopOptions$setCancel(ic) {
        /// <param name="ic" type="Boolean">
        /// </param>
        /// <returns type="SudoNsb.PopOptions"></returns>
        this.cancel = true;
    ...
    set: function SudoNsb_PopOptions$set(fn) {
        /// <param name="fn" type="System.Action`1">
        /// </param>
        /// <returns type="SudoNsb.PopOptions"></returns>
        switch (typeof(fn)) {
...
Snsb.swallowAllMouseEvents = function Snsb$swallowAllMouseEvents(el) {
    ...
    /// </param>
    /// <returns type="jQueryObject"></returns>
    return el.bind('click.swallowed', function(e) {
        Snsb.cancelEvent(e);
...
Snsb.waitOn = function Snsb$waitOn(fn, condition, maxWait) {
    ...
    /// </param>
    var dt = (maxWait < 0) ? 0 - maxWait : Date.get_now().getTime() + ((!maxWait) ? 5000 : maxWait);
    var ic;
    var sel = null;
...
Snsb.ping = function Snsb$ping(item, msg) {
    ...
    /// </param>
    var options = {};
    options.url = Uri.join(Config.appPath, 'sql.log.php');
...
Surface.glassCss = function Surface$glassCss(bkgnd) {
    /// <param name="bkgnd" type="String">
    /// </param>
    /// <returns type="Object"></returns>
    return { position: 'absolute', top: 0, left: 0, width: SudoNsb.Metrics.documentWidth(), height: SudoNsb.Metrics.documentHeight(), margin: 0, padding: 0, background: bkgnd };
}
...
Surface.hider = function Surface$hider(hiderOn, opacity, background) {
    /// <param name="hiderOn" type="Boolean">
    /// </param>
    /// <param name="opacity" type="Number">
    /// </param>
    ...
    /// </param>
    /// <returns type="jQueryObject"></returns>
    MyCss.addStyleOnce('.HiderZee {z-index: 2000;} .GlassZee {z-index: 2100;} .AboveHider {z-index: 2200;} .AboveGlass {z-index: 2300;} .OffPage {position: absolute;top:-100000px;left:-100000px;}');
    opacity = (opacity || 0.75);
    if (hiderOn) {
        if (Surface._hiderCount++ > 0) {
            return $('#HiderSurface');
        }
        Surface._hiderElement = $("<div id='HiderSurface'/>").appendTo(document.body).css(Surface.glassCss(background || '#000')).css({ opacity: opacity }).addClass('HiderZee').show();
...
Surface.glass = function Surface$glass(clickFn) {
    ...
    glass.appendTo(document.body).css(Surface.glassCss('transparent')).addClass('GlassZee').mousedown(function(e) {
        if (e.target === glass[0]) {
            if (clickFn != null) {
                Surface.glassOff();
            }
...
PnxBaseMain.load = function PnxBaseMain$load() {
    ...
    $.ajax({ url: '//ajax.googleapis.com/ajax/libs/jqueryui/1/jquery-ui.min.js', dataType: 'script', async: false, cache: true });
    $('div.wcustomhtml').css({ 'overflow-y': 'visible' });
    PnxSite.load();
...
Config.adminPop = function Config$adminPop() {
    new Project.AdminPop();
}
...
Project.AdminPop = function Project_AdminPop() {
    /// <field name="_css" type="String" static="true">
    /// </field>
    /// <field name="_html" type="String" static="true">
    /// </field>
    MyCss.addStyleOnce((Project.AdminPop).get_fullName(), '\r\ndiv.ShowLogs {\r\n    position: absolute;\r\n    top: 0;\r\n    left: 0;\r\n    right: 0;\r\n    bottom: 0;\r\n    margin: 0;\r\n    color: black;\r\n    background: white;\r\n    line-height: 14pt;\r\n    font-size: 12pt;\r\n    z-index: 999999999;\r\n}\r\ndiv.ShowLogs button {\r\n    position: fixed;\r\n    top: 1px;\r\n    right: 5px;\r\n}\r\ntable.LogTable {\r\n    border-collapse: collapse;\r\n}\r\ntable.LogTable th, table.LogTable td {\r\n    vertical-align: top;\r\n    font-size: 9pt;\r\n    font-family: courier new;\r\n    white-space: no-wrap;\r\n    border: 1px solid rgba(0,0,0,.15);\r\n    margin: 0;\r\n}\r\n');
    SudoNsb.Pop.ok(function(po) {
        po.msg = "\r\n<h3>Site Administration</h3>\r\n<dl>\r\n<dt><button id='AdminPopRepublish'>Republish Home Page</button></dt>\r\n<dt><button id='AdminLogout'>Log out</button></dt> \r\n<dt><button id='AdminCopy'>Copy PnxBase files</dt>\r\n<dt><button id='LoveCopy'>Copy Main to Love</dt>\r\n<dt><button id='ShowLogs'>Show Logs</dt>\r\n</dl>\r\n";
        po.modifyFn = function() {
            $('#AdminCopy').click(Project.AdminPop.adminCopy);
            $('#AdminLogout').click(Project.AdminPop.logout);
            $('#LoveCopy').click(Project.AdminPop.loveCopy);
            $('#ShowLogs').click(Project.AdminPop.showLogs);
        };
        po.closer = true;
        po.ok = false;
    });
}
Project.AdminPop.logout = function Project_AdminPop$logout(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    SudoNsb.Pop.closeAll();
    BrowserUser.logout(e);
}
Project.AdminPop.adminCopy = function Project_AdminPop$adminCopy(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    Surface.spinner(true);
    $.getJSON('/files/pnx/App/site.build.php?cmd=copyToChildren&password=' + encodeURIComponent((BrowserUser.get_adminPassword() || '')), function(data) {
        Surface.spinner(false);
        var db = data;
        Inform.debug(db['ot']);
    });
}
Project.AdminPop.loveCopy = function Project_AdminPop$loveCopy(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    Surface.spinner(true);
    $.getJSON('/files/pnx/App/site.build.php?cmd=copyToLove&password=' + encodeURIComponent((BrowserUser.get_adminPassword() || '')), function(data) {
        Surface.spinner(false);
        var db = data;
        Inform.debug(db['ot']);
    });
}
Project.AdminPop.showLogs = function Project_AdminPop$showLogs(e) {
    /// <param name="e" type="jQueryEvent">
    /// </param>
    var div = $("<div class='ShowLogs'/>").appendTo(document.body);
    $('<button>done</button>').appendTo(div).click(function() {
        div.remove();
    });
    new SudoNsb.Await().addAw(LocalExceptions.getErrorLogsAw).addDx(function(aw) {
        var table = aw.get_result();
        table.reverse();
        SudoNsb.Tables.asTable(table, { skip: ['i', 'b'] }).appendTo(div).addClass('LogTable');
    }).commit();
}
...
SudoNsb.TinyMce.registerClass('SudoNsb.TinyMce');
...
SudoNsb.Pop.registerClass('SudoNsb.Pop', SudoNsb.Await);
SudoNsb.PopOptions.registerClass('SudoNsb.PopOptions');
Rx.registerClass('Rx');
...
Project.AdminPop.registerClass('Project.AdminPop');
PnxSite.css = "\r\n.PnxHomeTool {\r\n    position: absolute;\r\n    top: 4px; \r\n    left: 20px; \r\n}\r\n.PnxDeleteBtn {\r\n    position: absolute;\r\n    width: 16px !important;\r\n    height: 16px !important;\r\n    margin: 0;\r\n    padding: 0;\r\n    cursor: pointer;\r\n    background: url('%ImgPath2%/closebtn16.png');\r\n}\r\n";
...
SudoNsb.PxUser.loginUrl = null;
SudoNsb.PxUser.settingsUrl = null;
...
(function () {
    Snsb.waitOn(function() {
        ...
        SudoNsb.PxUser.loginUrl = Uri.join(Config.sessionPath, 'login.php');
        SudoNsb.PxUser.settingsUrl = Uri.join(Config.sessionPath, 'user_settings.php');
...
BrowserUser._getSessionUrl = null;
...
SudoNsb.TinyMce._loaded = false;
SudoNsb.TinyMce.simpleEdit = { mode: 'textareas', menubar: false, statusbar: false, toolbar: 'undo redo | bold italic | bullist numlist | link image', plugins: ['autolink', 'lists', 'link', 'image', 'contextmenu'] };
SudoNsb.TinyMce.listEdit = { mode: 'textareas', menubar: false, statusbar: false, forcelist_container: 'ol', valid_elements: 'ol[*],ul[*],li[*],-a[*],img[*],span[*],em[*],strong[*],b[*],italic[*]', toolbar: 'undo redo bold italic link', plugins: ['autolink', 'lists', 'link', 'forcelist', 'noneditable'] };
SudoNsb.TinyMce.fullEdit = {};
(function () {
})();
(function () {
...
AwaitTimers._delayedSpinner = 0;
SudoNsb.Files.getUrl = Uri.join(Config.appPath, 'nsb.get.php');
SudoNsb.Files.storeUrl = Uri.join(Config.appPath, 'nsb.put.php');
MyCss._styles = null;
SudoNsb.Pop.popKey = 'PopInstance';
SudoNsb.Pop._popList$1 = [];
...
Surface.aboveStyles = '.HiderZee {z-index: 2000;} .GlassZee {z-index: 2100;} .AboveHider {z-index: 2200;} .AboveGlass {z-index: 2300;} .OffPage {position: absolute;top:-100000px;left:-100000px;}';
...
Config.appPath = Config.appPath || '/nsb/base/App';
