...
class loggedInUser {
	...
	public function updateLastSignIn()
	{
		...
		$stmt->execute();
		$stmt->close();	
		$this->updateFacebookId();
	}
	
	//Simple function to update the facebook id
	public function updateFacebookId()
	{
		global $mysqli,$db_table_prefix,$fbid;
		if (!empty($fbid)) {
			$this->$fbid = $fbid;
			$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
				SET
				...
				WHERE
				id = ?");
			$stmt->bind_param("si", $fbid, $this->user_id);
			$stmt->execute();
			$stmt->close();	
		}
	}
